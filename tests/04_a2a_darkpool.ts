...
// ============================================================
// A2A Dark Pool — full integration test with CPIs to vault + engine
// ============================================================
// Depends on 01_perp_vault.ts having initialized vault_config + usdc mint,
// and 02_perp_engine.ts having initialized engine_config + added BTC-USD
// market. This test:
//   1) Authorizes darkpool_authority PDA as operator in BOTH vault + engine
//   2) Has intentCreator + responder + feeRecipient deposit USDC into vault
//   3) Initializes darkpool config with real engine + vault program IDs
//   4) Runs full intent → response → settle flow with all CPIs
//   5) Verifies positions opened in engine + USDC fee moved in vault
...
describe("a2a_darkpool", () => {
  ...
  before(async () => {
    // Airdrop SOL to traders + fee_recipient (need SOL for SPL operations + signing).
    ...
    // Pull the canonical USDC mint from vault_config (set by 01_perp_vault).
    ...
    const seed = 1_000 * 1_000_000; // $1,000 each
    for (const kp of [intentCreator, responder, feeRecipient]) {
  ...
  it("posts an intent (BUY 1 BTC at $50,000-50,200)", async () => {
    const intentId = new anchor.BN(1);
    await program.methods
      .postIntent(
        Array.from(marketIdBtc),
        true,                                  // is_buy
        new anchor.BN(1 * 100_000_000),        // size = 1 BTC (matches $50K which is < $10K threshold... wait notional = 50K * 1 = $50K, which IS large)
  ...
  it("posts a response", async () => {
    ...
    await program.methods
      .postResponse(
        new anchor.BN(50_000 * 1_000_000),  // price = $50,000
        new anchor.BN(600),
      )
      .accounts({
        config: configPda,
  ...
  it("settles via CPI: opens positions in engine + moves fee in vault", async () => {
    ...
    // Verify intent + response status flipped
    const intent = await program.account.intent.fetch(intentPda(intentId));
    ...
    // Verify positions opened in engine
    ...
    // Verify fee moved in vault
    // fee = notional * fee_bps / BPS = (50K * 1) * 3 / 10000 = $15 = 15_000_000 (6 decimals)
    const expectedFee = 15_000_000;
    ...
    assert.equal(
      buyerBalanceAfter,
      Number(buyerBalanceBefore) - expectedFee,
      "buyer balance debited by fee",
    );
    assert.equal(
      sellerBalanceAfter,
      Number(sellerBalanceBefore) - expectedFee,
      "seller balance debited by fee",
    );
    assert.equal(
      feeRecipientAfter,
      Number(feeRecipientBefore) + 2 * expectedFee,
